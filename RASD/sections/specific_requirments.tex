...
\begin{itemize}
    \item \textbf{Maps}: to implement the functionality allowing the Users to look for a store the system will interface with a digital map service. The Store Manager will be able to register on the map the position of their store.
    \item \textbf{Localization}: localization will be used in the webapp and in the mobile app in order to show the User their current position, helping them to find their desired store.
    \item \textbf{Mobile OS Notification Service}: notifications will be sent to warn and update continuously Users about their Reservations (if any) and the status of the Virtual Line (if it is joined).
    \item \textbf{SMS service}: an SMS with a secret code will be sent to the user upon login in order to verify their identity.
\end{itemize}

\subsubsection{Communication Interfaces}
\todo{improve?}The backend of the system will expose a unified REST compliant API to communicate with all clients using HTTPS and TCP/IP. The User will need an internet connection as well as cellular network upon login. The Totem will be provided with WiFi or cellular connectivity, depending on the needs of each store.
