...
\begin{enumerate}[label={[G\arabic*]}]
    ...
    \item allow managers to have control over the maximum number of people in a store by impeding the entrance when the maximum number is reached until someone exits the store
    \item allow managers to monitor the current status of the store (for example, number of people in the store, fullness of the queues and slots ) 
    \item allow managers to get insights on the behaviour of the users by registering the enter/exit time
\end{enumerate}
